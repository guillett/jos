...
describe 'mapping of articles' do
  ...
  describe "when we have one article" do
    ...
    before do
      @xml = Nokogiri.Slop(fake_article_file({  id_article_origin: "LEGIARTI000006204293",
                                                state: "VIGUEUR",
                                                start_date: "2006-09-01",
                                                end_date: "2999-01-01",
                                                nota_content: "Décret n° 2006-975 du 1er août 2006 art. 8 : I.-Les dispositions du présent décret entrent en vigueur le 1er septembre 2006.",
                                                text_content: "I.-Les dispositions du présent code s'appliquent aux marchés publics et aux accords-cadres ainsi définis :",
                                                link_id: "LEGIARTI000017843672",
                                                link_title: "Code des marchés publics - art. 150 (V)"
                                             },
                                            1))
      @articleMap = ArticleMap.parse(@xml, :single => true)
    end

    it 'maps correctly the article' do
      expect(@articleMap.ID).to           eq("LEGIARTI000006204293")
      expect(@articleMap.nota).to         eq("Décret n° 2006-975 du 1er août 2006 art. 8 : I.-Les dispositions du présent décret entrent en vigueur le 1er septembre 2006.")
      expect(@articleMap.bloc_textuel).to eq("I.-Les dispositions du présent code s'appliquent aux marchés publics et aux accords-cadres ainsi définis :")
